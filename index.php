...
<script>
myVar=window.int=self.setInterval(function(){onTimerUpdate("top5")},4000);
showUser("top5");
</script>
...
//	echo "</form>";
	echo "<br />";
	echo "<label for='dummy1' id='logoutHeader'><a href='index1.php'>Sign out</a></label>";
	echo "</span>";
	echo "</div>";
?>
		<ul id="nav">
<!-- mtf html -->
		...
		<li class="active"><a href="index.php">Sportsbook</a>
			<ul>
...
<!-- Same as above, probably with betting games  -->
		...
		<div id="content">
			...
			<br />	
			<h1>No Juice, No Takeout, No Vig, No Take, No House Edge!</h1>
...
<div id="txtHint3"><b>Most are engaged in business the greater part of their lives, because the soul abhors a vacuum and they have not discovered any continuous employment for man's nobler faculties.</b></div>
<form>
...
<button onclick="myVar=window.int=self.setInterval(function(){onTimerUpdate("top5")},4000)">Starty</button>
<br />
<input type="text" id="clock" />
<button onclick="myVar=window.clearInterval(myVar)">Stoppy</button>
<br />
<?php
	echo $_SESSION['SESS_PLAYER_ACCT_ID'];
?>
<?php
	echo $_SESSION['SESS_POOL_ACCT_ID'];
?>
<?php
	echo $_SESSION['SESS_AMOUNT'];
?>
<?php
	echo $_SESSION['SESS_PICK_ROT_ID'];
?>
